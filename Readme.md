...
**Важно:** `location /` в самом конце должен быть именно **`proxy_pass`** на контейнер `server`, а не `root` + `try_files $uri $uri/ /index.html`, отдающий статику с диска. Собирать клиент отдельно вручную и класть его в папку сайта (`/www/wwwroot/...` и т.п.) **не нужно** — актуальный билд `client/` уже лежит внутри образа `server` (см. `server/Dockerfile`, стадия `client-build` → `server/public`) и раздаётся самим Node-процессом на порту `3003`. Если отдавать статику напрямую из nginx-папки, придётся вручную пересобирать и копировать клиент туда при каждом обновлении кода, и версии клиента/сервера могут незаметно разойтись — просто проксируйте `/` в контейнер, как и остальные `location`, и после `docker compose build server && docker compose up -d` сайт всегда будет актуальным сам по себе.
