...
**После любой правки кода** пересобирайте образ явно через SSH-терминал, а не кнопками «Удалить»/«Создать» в интерфейсе aaPanel — они пересоздают контейнер из уже существующего образа, без пересборки:

```bash
docker compose build server --no-cache
docker compose build maxbot --no-cache
docker compose up -d
```
